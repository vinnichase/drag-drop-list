...
const useChildrenHeights = (children, onHeightChange) => {
    ...
    const _fnSprings = useCallback(index => ({
        y: getItemYPos(index),
    ...
    return {
        ...
        setDragEnd: (
            ...
            y: getItemYPos(index, true),
            shadow: 0,
            zIndex: '0',
            onRest: () => {
                if (currentIndex.current !== newIndex.current) {
                    onFinish(currentIndex.current, newIndex.current);
                }
                resetDragVars();
            },
        })),
...
export const DragList = ({
    children,
    onMove = (fromIndex = 0, toIndex = 0) => [fromIndex, toIndex],
}) => {
    console.log('render');
